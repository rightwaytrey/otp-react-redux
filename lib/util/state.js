...
/**
 * Backlog 14.2, rider's answer (b) 2026-09-26. The 00:02 Friday-night search
 * came back with one route-4 card and a bike card; route 4 was the only real
 * transit between those points for four hours, and 14.2's 14400 top-up
 * (`widenThinSearch`, actions/apiV2.js) had already fired and found only more
 * route-4 departures. Nothing on screen said why the list was short: the
 * window-ran-dry code is hidden by metro-error-renderer on purpose.
 *
 * So: a settled, non-Go-Mode search that asked for transit and holds at least
 * one but fewer than THIN_TRANSIT_ITINERARY_COUNT distinct transit
 * itineraries — the same count, over the same stored responses, that decides
 * the top-up — gets one line saying few routes run. `pending` covers the
 * top-up in flight (ROUTING_EXTRA_REQUEST increments it). Zero transit is
 * 22.2's NO_TRANSIT_OPTION_FOUND, and any combination that errored means the
 * short list is the server's doing, not the timetable's, so it says nothing.
 * Not gated by clock hour: the threshold is time-agnostic, and "at this hour"
 * is true of whatever hour the rider asked about.
 */
export function searchIsThinOnTransit(state, activeSearch) {
  if (!activeSearch || activeSearch.pending) return false
  const responses = activeSearch.response || []
  if (!responses.some((res) => res?.transitRequested)) return false
  if (responses.some((res) => res?.error)) return false
  const goMode = state?.otp?.goMode
  if (
    goMode?.isActive ||
    (goMode?.reRoute?.searchId &&
      goMode.reRoute.searchId === state?.otp?.activeSearchId)
  ) {
    return false
  }
  const transitCount = countTransitItineraries(responses)
  return transitCount > 0 && transitCount < THIN_TRANSIT_ITINERARY_COUNT
}

export function getActiveSearchErrors(state) {
  const activeSearch = getActiveSearch(state)
  if (!activeSearch) return {}
  const errorsByCode = activeSearch?.response?.reduce((acc, cur) => {
  ...
  if (itineraries.length === 0 && !activeSearch?.pending) {
    additionalErrors.NO_TRANSIT_CONNECTION = new Set()
  }
  // Backlog 22.2. A settled search that DID run OTP's transit search, came
  // back with options, and has not one transit leg among them. The rider's
  // 2026-09-21 09:25:58 Bloomington -> Lakeville search was exactly this: one
  // 25 km bike card, and the only thing the panel said about transit was the
  // walk-only call's NO_STOPS_IN_RANGE — "Destination is not near any transit
  // stops", which nine probes against production that same morning showed to
  // be false (249 served stops within 15 km, nearest 5.2 km, and a
  // bike -> Orange Line -> bike chain at 74 min). So the panel says what is
  // true of the search and nothing about the address.
  //
  // `transitRequested` is stamped on each stored response by routingQuery's
  // rewritePayload; without it a rider who simply turned transit off would be
  // told no transit was found for a search that never looked.
  if (
    itineraries.length > 0 &&
    !activeSearch?.pending &&
    activeSearch?.response?.some((res) => res?.transitRequested) &&
    !itineraries.some((itin) =>
      (itin?.legs || []).some((leg) => coreUtils.itinerary.isTransitLeg(leg))
    )
  ) {
    additionalErrors.NO_TRANSIT_OPTION_FOUND = new Set()
  }
  if (searchIsThinOnTransit(state, activeSearch)) {
    additionalErrors.FEW_TRANSIT_ROUTES = new Set()
  }
  if (activeSearch?.response?.some((res) => res.error)) {
    // Log detailed error information for debugging
    activeSearch.response.forEach((res, index) => {
      if (res.error) {
        console.error(
          `[OTP Routing Error #${index}]`,
          '\nError object:',
          res.error,
          '\nRequest URL:',
          res.url,
    ...
    additionalErrors.SYSTEM_ERROR = new Set()
  }

  return { ...errorsByCode, ...additionalErrors }
}
...
/**
 * From a set of re-route candidates, pick the one that keeps the rider on the
 * route they already chose: first transit leg on the given route, earliest
 * departure first (the "next bus", not the shortest trip). Returns null when
 * nothing boards that route — the caller should fall back to asking the rider
 * rather than forcing a route they didn't pick.
 */
export function pickSameRouteReroute(itineraries, routeId) {
  if (!routeId) return null
  const firstTransitLeg = (it) => (it.legs || []).find((l) => l.transitLeg)
  const legRouteId = (leg) =>
    leg?.route?.gtfsId || leg?.route?.id || leg?.routeId || null
  return (
    itineraries
      .filter((it) => legRouteId(firstTransitLeg(it)) === routeId)
      .sort(
        (a, b) =>
          Number(firstTransitLeg(a).startTime) -
          Number(firstTransitLeg(b).startTime)
      )[0] || null
  )
}

/**
 * From a set of quiet-replan candidates (rider drifted off a WALK/BICYCLE
 * access leg), pick the itinerary to swap in silently — or null to leave the
 * trip alone. Never forces a route change:
 * - With a transit boarding still ahead, only an itinerary that boards the
 *   SAME route qualifies (next departure, via pickSameRouteReroute).
 * - With no transit left (pure walk/bike to the destination), the fastest
 *   all-WALK/BICYCLE itinerary qualifies; when the rider is on a BICYCLE leg
 *   it must still include a bike leg (never silently downgrade to a long walk).
 */
export function pickAccessReplanCandidate(
  itineraries,
  { accessMode, nextTransitRouteId } = {}
) {
  if (!itineraries?.length) return null
  if (nextTransitRouteId) {
    return pickSameRouteReroute(itineraries, nextTransitRouteId)
  }
  const accessOnly = itineraries.filter(
    (it) =>
      (it.legs || []).length > 0 &&
      (it.legs || []).every(
        (l) => !l.transitLeg && ['BICYCLE', 'WALK'].includes(l.mode)
      ) &&
      (accessMode !== 'BICYCLE' ||
        (it.legs || []).some((l) => l.mode === 'BICYCLE'))
  )
  return accessOnly.sort((a, b) => a.duration - b.duration)[0] || null
}
 ...
 */
export const isFilterMatch = (string, query) => {
...
/**
 * Add a search entry to the front of the given list, removes existing entries based on `name` field.
 */
export function addToSearches(searches, entry) {
  const { name } = entry
  return [entry, ...searches.filter((s) => s.name !== name)]
}
